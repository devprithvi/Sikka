...
import com.prithvi.sikka.style.SikkaTheme
...
@Composable
@Preview
fun App(viewModel: ExpenseViewModel) {
    val state by viewModel.stateI.collectAsState()
    ...
    SikkaTheme {
        Scaffold(
            topBar = {
                TopAppBar(
                    title = { Text("Sikka") },
                    actions = {
                        Button(onClick = { viewModel.deleteAllExpensesI() }) {
                            Text("Delete All")
                        }
                    }
                )
        ...
        showDeleteDialog?.let { expense ->
            DeleteConfirmationDialog(
                expense = expense,
                onConfirm = {
                    viewModel.deleteExpenseI(expense.id)
                ...
                onClick = {
                    ...
                    if (expenseAmount != null && description.isNotBlank()) {
                        val newExpense = ExpenseEntity(
                            ...
                            createdAt = expense?.createdAt ?: Clock.System.now()
                                .toEpochMilliseconds()
                        )
                        onSave(newExpense)
                    }
